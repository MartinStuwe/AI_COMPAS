...
def pool_observation(PAR: dict, observation_in_pixel, resample=False):
    ...
    # resampling? then highest granularity (=72) by default
    if resample:
        convolutionGranularity = 72
    else:
...
def select_action_goal(PAR: dict, HL_SoC: float, observation_in_pixel, reference: tuple,
    ...
    zero_coords = np.argwhere(observation_np == 1) # Try to change to 0 which is task driven i.e. top down, 1 bottom up 
